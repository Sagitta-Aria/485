...
#define WIFI_SERVER_RX_BUFFER_SIZE       512U   // TCP 单行接收、命令和回复缓冲区大小
...
/*
 * 在调用者持有发送锁时写完一整段 TCP 数据；不能在 ISR 中调用。
 * 返回 false 表示连接已经不能继续发送，调用者应等待重连。
 */
static bool wifi_server_write_locked(int socket_fd, const char *data, size_t length)
{
    ...
    while (sent < length) {
        ...
        if (result <= 0) {
            ...
            /* A partial line must never be followed by another protocol frame. */
            if (s_connection_socket == socket_fd) s_connection_socket = -1;
...
static bool wifi_server_send_all(int socket_fd, const char *data, size_t length)
{
    if (s_socket_mutex == NULL ||
        xSemaphoreTake(s_socket_mutex, portMAX_DELAY) != pdTRUE) return false;
    bool sent = wifi_server_write_locked(socket_fd, data, length);
    xSemaphoreGive(s_socket_mutex);
    return sent;
}

/* Keep socket lookup and the complete write under the same close/reconnect lock. */
static esp_err_t wifi_server_send_current(const char *data, size_t length)
{
    if (s_socket_mutex == NULL ||
        xSemaphoreTake(s_socket_mutex, portMAX_DELAY) != pdTRUE) return ESP_ERR_INVALID_STATE;
    esp_err_t result = s_connection_socket < 0 ? ESP_ERR_INVALID_STATE :
        (wifi_server_write_locked(s_connection_socket, data, length) ? ESP_OK : ESP_FAIL);
    xSemaphoreGive(s_socket_mutex);
    return result;
}
...
static void wifi_server_handle_status(int socket_fd)
{
    ...
    wifi_server_format_status_hex(status.chip[CH446_CHIP_U1],
                                  ...
                                  s1_hex,
                                  sizeof(s1_hex));
    wifi_server_reply(socket_fd, "OK STATUS S1 %s", s1_hex);
}

/* Return one structured XD31H diagnostic without collapsing its root cause. */
static void wifi_server_reply_xd31h_error(
    int socket_fd,
    const char *operation,
    const xd31h_measurement_t *measurement,
    esp_err_t error)
{
    char diagnostic[XD31H_DIAGNOSTIC_TEXT_SIZE];
    xd31h_format_diagnostic(measurement, error,
                            diagnostic, sizeof(diagnostic));
    wifi_server_reply(socket_fd, "ERR %s %s", operation, diagnostic);
}

/*
 * 执行一次 XD31H 测量并通过当前 TCP 连接返回结果。
 * 只能在 xd31h_init() 成功后调用；本函数不会建立或复位 CH446 通路。
 */
static void wifi_server_handle_measure(int socket_fd)
{
    xd31h_measurement_t measurement; // 保存状态、量程、原始值和换算后的电阻值
    const esp_err_t error = xd31h_read_measurement(
        &measurement, XD31H_DEFAULT_TIMEOUT_MS);

    // Transport/parser failures and XD31H status errors share one detailed path.
    if (error != ESP_OK || !measurement.valid) {
        wifi_server_reply_xd31h_error(socket_fd, "MEASURE",
                                      &measurement, error);
        return;
    }

    wifi_server_reply(socket_fd,
                      "OK MEASURE resistance=%.3f raw=%u range=%u",
...
static void wifi_server_handle_connect(int socket_fd, const char *command)
{
    ...
    const esp_err_t error = ch446_connect_kelvin_pair(
        pair.positive_port, pair.negative_port);
    if (error != ESP_OK) {
...
static void wifi_server_handle_switch(int socket_fd, const char *command)
{
    ...
    const esp_err_t error = ch446_set_port_bus(port, bus, closed);
    if (error != ESP_OK) {
...
/* Forward a legacy matrix command to one numeric RS485 slave ID. */
static void wifi_server_handle_bus(int socket_fd, const char *command)
{
    char target_id[WIFI_SERVER_ID_SIZE];
    char payload[WIFI_SERVER_RX_BUFFER_SIZE];
    char extra;
    const int field_count = sscanf(command,
                                   "BUS %31s %255[^\n] %c",
                                   target_id,
                                   payload,
                                   &extra);
    if (field_count != 2 || payload[0] == '\0') {
        wifi_server_reply(socket_fd,
                          "ERR BUS usage=BUS slave1|slave2|slave3|slave4|broadcast COMMAND");
        return;
    }

    char result[WIFI_SERVER_RX_BUFFER_SIZE];
    (void)rs485_master_execute(target_id,
                               payload,
                               result,
                               sizeof(result));
    wifi_server_reply(socket_fd, "%s", result);
}

/*
 * 把一条已经完成 TCP 分帧的本地硬件命令分发给对应处理函数。
 * 处理本机 MEASURE 和矩阵命令，并支持 BUS <slave> <command> 的 RS485 转发。
...
static void wifi_server_handle_command(int socket_fd, char *raw_command)
{
    ...
    if (strncmp(command, "TOPO_", 5U) == 0) {
        const esp_err_t error = s_result_context_active ?
            topology_scan_submit(command, s_result_source, s_result_request_id) :
            ESP_ERR_INVALID_ARG;
        if (error != ESP_OK) {
            wifi_server_reply(socket_fd, "ERR TOPO %s", esp_err_to_name(error));
        }
        return;
    }
    bool hardware_command = strcasecmp(command, "MEASURE") == 0 ||
        strcasecmp(command, "RESET") == 0 || strncasecmp(command, "CONNECT ", 8) == 0 ||
        strncasecmp(command, "SWITCH ", 7) == 0 || strncasecmp(command, "BUS ", 4) == 0;
    if (hardware_command && topology_scan_debug_lock() != ESP_OK) {
        wifi_server_reply(socket_fd, "ERR BUSY TOPOLOGY_SESSION");
        return;
    }

    // RESET 清理选路；master1 保留四路固定 Kelvin 连接，master2 全部断开。
    ...
    if (strcasecmp(command, "PING") == 0) {
        ...
        const esp_err_t error = ch446_reset_all();
        if (error == ESP_OK) {
    ...
    } else if (strncasecmp(command, "BUS ", 4U) == 0) {
        wifi_server_handle_bus(socket_fd, command);
    } else if (strcasecmp(command, "HELP") == 0) {
        wifi_server_reply(socket_fd,
                           "OK HELP PING|STATUS|MEASURE|RESET|CONNECT S1 0 S2 0|SWITCH S1 0 Y4 ON|OFF|BUS slave1|broadcast COMMAND|HELP");
    } else {
        wifi_server_reply(socket_fd, "ERR UNKNOWN_COMMAND");
    }
    if (hardware_command) topology_scan_debug_unlock();
}
        ...
        for (int index = 0; index < received; index++) {
            ...
            if (byte == '\n') {
                ...
                if (overflow) {
                    ...
                    } else if (strncmp(command, "RESULT ", 7U) == 0) {
                        char sender[32], destination[32], request_id[32], payload[320];
                        if (sscanf(command, "RESULT %31s %31s %31s %319[^\n]",
                                   sender, destination, request_id, payload) == 4) {
                            topology_scan_feed_result(sender, destination, request_id, payload);
                        }
                                   ...
                                   const char *payload)
{
    ...
    return wifi_server_send_current(request, (size_t)length);
}

esp_err_t wifi_server_send_result(const char *destination,
                                 const char *request_id,
                                 const char *payload)
{
    if (!wifi_server_valid_token(destination) || !wifi_server_valid_token(request_id) ||
        !payload || strchr(payload, '\n') || strchr(payload, '\r')) return ESP_ERR_INVALID_ARG;
    char line[WIFI_SERVER_RX_BUFFER_SIZE];
    int length = snprintf(line, sizeof(line), "RESULT %s %s %s %s\n",
                          WIFI_DEVICE_ID, destination, request_id, payload);
    if (length < 0 || (size_t)length >= sizeof(line)) return ESP_ERR_INVALID_SIZE;
    return wifi_server_send_current(line, (size_t)length);
}
...
static void wifi_server_task(void *argument)
{
    ...
    while (true) {
        ...
        /* HELLO must precede all worker SEND/RESULT frames on a new connection. */
        xSemaphoreTake(s_socket_mutex, portMAX_DELAY);
        s_connection_socket = socket_fd;
        xSemaphoreGive(s_socket_mutex);
