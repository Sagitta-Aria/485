...
/* Claim under test: erasing the alternate metadata bank before the next
 * generation is committed leaves a window with no readable metadata at all,
 * which would let recovery discard unacknowledged records. Cut power at every
 * byte of the bank switch and require the record to survive. */
EXPORT int journal_test_metadata_switch_keeps_unacked_records(void)
{
    for (int cut = 0; cut < 160; ++cut) {
        CHECK(fresh() == ESP_OK);
        uint32_t sequence, crc;
        CHECK(append(&sequence, &crc) == ESP_OK);
        /* Fill the current bank so the next save_meta() must switch banks. */
        journal_meta_t candidate = s_meta;
        while ((s_meta_offset % SECTOR_SIZE) < SECTOR_SIZE - sizeof(candidate))
            CHECK(save_meta(candidate) == ESP_OK);
        uint32_t survivor, survivor_crc;
        CHECK(append(&survivor, &survivor_crc) == ESP_OK);

        /* Cut power exactly across the bank switch (erase + first commit). */
        write_budget = erase_budget = -1;
        erase_budget = cut;
        write_budget = cut;
        const esp_err_t switched = save_meta(candidate);
        if (switched == ESP_OK) continue;

        /* Whatever happened, recovery must still expose the unacked record. */
        CHECK(reboot() == ESP_OK);
        topology_journal_record_t record;
        CHECK(topology_journal_read(survivor, &record) == ESP_OK);
        CHECK(record.crc == survivor_crc);
        CHECK(topology_journal_ack(survivor, survivor_crc) == ESP_OK);
        CHECK(!nor_errors);
    }
    return 0;
}
