...
/* Modbus RTU delimits frames with an idle line of at least 3.5 character times;
 * at 115200 8N1 one character is about 87 us, so this is roughly 304 us. */
#define RS485_INTERFRAME_GAP_US 304U
...
/* Smallest positive delay on this tick rate already exceeds 3.5 characters. */
static TickType_t rs485_silence_ticks(void)
{
    const TickType_t ticks = pdMS_TO_TICKS((RS485_INTERFRAME_GAP_US + 999U) / 1000U);
    return ticks == 0 ? 1 : ticks;
}
...
/* A frame only ends after the line has been idle. Without this check a burst of
 * line noise containing 0xA5 makes this call return in the middle of the noise,
 * and the caller's next frame is then assembled from misaligned bytes. */
static esp_err_t rs485_await_interframe_gap(void)
{
    uint8_t extra = 0U;
    const int count = uart_read_bytes(BOARD_RS485_UART_PORT, &extra, 1U,
                                      rs485_silence_ticks());
    if (count > 0) {
...
esp_err_t rs485_bus_receive(rs485_frame_t *frame, uint32_t timeout_ms)
{
    ...
    const TickType_t start = xTaskGetTickCount();
    /* Every read is bounded by what is left of the caller's deadline, so the
     * inter-frame gap can never push one call past its requested timeout. */
    const TickType_t deadline = start + timeout_ticks;
    uint8_t byte = 0U;
    while (true) {
        const TickType_t now = xTaskGetTickCount();
        if ((TickType_t)(now - start) >= timeout_ticks) {
            return ESP_ERR_TIMEOUT;
        }
    ...
    uint8_t header[3];
    {
        ...
        esp_err_t error = rs485_read_exact(header, sizeof(header),
                                           (TickType_t)(deadline - now));
        if (error != ESP_OK) {
            return error;
        }
    ...
    uint8_t tail[RS485_MAX_DATA_LENGTH + 2U];
    {
        ...
        esp_err_t error = rs485_read_exact(tail, frame->length + 2U,
                                           (TickType_t)(deadline - now));
        if (error != ESP_OK) {
            return error;
        }
    ...
    /* Bytes arriving during the gap mean this was a fragment, not a frame. */
    return rs485_await_interframe_gap();
}
